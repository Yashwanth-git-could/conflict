def team_toss_lost():
    ...
    cursor=mydb.cursor()
    cursor.execute('select * from team_lost_toss')
    ...
    plt.bar(index + barwidth, z, barwidth, label='Won', color='lightgreen')  # Matches won
    plt.bar(index + 2 * barwidth, w, barwidth, label='Lost', color='red')  # Matches lost
    ...
    plt.title('Toss and Triumph: Exploring Wins and Losses of IPL Teams (Toss Lost)',color='darkblue')
